...
class StubLLMClient:
    ...
    def create_with_tools(self, *, model, messages, tools, tool_choice=None, parallel_tool_calls=True):
        self.calls.append(
            {
                "kind": "plan",
                "model": model,
    ...
    def get_response(self, model, messages, stream=False):
        self.calls.append({"kind": "summary", "model": model, "messages": messages})
...
def test_tool_agent_flow_executes_tools(tmp_path):
    ...
    flow = create_tool_agent_flow(registry=registry, llm_client=stub)
...
def test_tool_agent_flow_direct_response(tmp_path):
    ...
    flow = create_tool_agent_flow(registry=create_default_registry(include=["read"]), llm_client=stub)
