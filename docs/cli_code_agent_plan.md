...
- CLI 驱动与输出：在 `cli.py` 中内联解析逻辑，调用 `run_code_agent_cli`，实时输出规划、工具和总结。
...
- [x] 保证默认注册全部工具。
- [x] 在 `cli.py` 内联 CLI 解析并调用 `run_code_agent_cli`。
- [x] 更新文档说明使用与测试命令。
...
- 入口：`cli.py` 内联解析，默认执行 `agent` 命令；`README` 提供运行示例。
...
- `ToolExecutionBatchNode`：`tool_plan.tool_calls` 驱动执行；`mode`/`parallel` 控制批次；`post` 将结果写入 `shared["tool_results"]` 与历史，为后续总结节点提供输入；`max_parallel_workers` 可调节并发限额；工具返回统一封装为 `ToolOutput` 对象（`inputs` + `{status, content, data}`）。
